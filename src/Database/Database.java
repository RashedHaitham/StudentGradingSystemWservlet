...
public class Database {
    ...
    public StudentGrades viewGrades(int studentIdInput) {
        ...
        String studentName = "";
        try {
            String query = "SELECT s.username AS student_name, c.course_id, c.course_name, g.grade " +
                    "FROM students s " +
                    "JOIN courses c ON c.course_id IN (SELECT course_id FROM grades WHERE student_id = s.student_id) " +
                    "LEFT JOIN grades g ON g.student_id = s.student_id AND g.course_id = c.course_id " +
                    "WHERE s.student_id = ?";
            try (ResultSet resultSet = executeQuery(query, studentIdInput)) {
                while (resultSet.next()) {
...
//Modified to accept a connection parameter for transaction management
